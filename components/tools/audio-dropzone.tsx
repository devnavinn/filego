...
import { AudioLines, RefreshCw, Upload, X } from "lucide-react"
...
export function AudioDropzone({
    ...
    if (file) {
        return (
            <div className={cn("space-y-3 rounded-2xl border border-border/60 bg-card p-4", className)}>
    ...
    return (
        ...
        >
            {isDragging ? <Upload className="h-6 w-6 text-muted-foreground" /> : <AudioLines className="h-6 w-6 text-muted-foreground" />}
