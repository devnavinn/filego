...
export async function POST(req: Request) {
    ...
    if (!userId) {
        return NextResponse.json(
            { ok: false, error: "Sign in to parse resumes with AI." },
            { status: 401 }
        )
    }
    ...
    if (text.length < MIN_CHARS || text.length > MAX_CHARS) {
        return NextResponse.json(
            { ok: false, error: `Provide resume text between ${MIN_CHARS} and ${MAX_CHARS.toLocaleString()} characters.` },
            { status: 400 }
        )
    }
    ...
    try {
        ...
        console.error("[AI_RESUME_PARSER_ERROR]", error)
        return NextResponse.json(
            { ok: false, error: "AI resume parsing failed. Please try again." },
            { status: 500 }
        )
    }
}
